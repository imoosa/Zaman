...
def create_app():
    ...
    @app.get("/api/mobile/calendar")
    def api_mobile_calendar():
        """Flutter calendar API using the same CALENDARS architecture as /calendar.

        Supports all primary/secondary calendar combinations already available
        in the web Settings page. The Flutter client is intentionally stateless:
        it sends cal/secondary and visible event sources with every request.
        """
        today_g = date.today()
        prefs = get_user_prefs()

        cal_key = request.args.get("cal") or prefs.get("default_calendar", "hijri")
        if cal_key not in CALENDARS:
            cal_key = "hijri"
        cal = CALENDARS[cal_key]

        secondary_cal = request.args.get("secondary")
        if secondary_cal is None:
            secondary_cal = prefs.get("secondary_calendar", "")
        show_secondary = bool(
            secondary_cal and secondary_cal in CALENDARS and secondary_cal != cal_key
        )
        secondary_obj = CALENDARS[secondary_cal] if show_secondary else None

        # Calendar coordinates can be supplied explicitly by Flutter. Otherwise
        # use the same saved/default location as the web application.
        loc = current_location()
        try:
            lat = float(request.args.get("lat")) if request.args.get("lat") is not None else loc["lat"]
            lng = float(request.args.get("lng")) if request.args.get("lng") is not None else loc["lng"]
            tz_offset = float(request.args.get("tz_offset")) if request.args.get("tz_offset") is not None else loc["tz_offset"]
        except ValueError:
            raise ValueError("lat, lng and tz_offset must be numbers")

        ty, tm, _ = cal["native_of"](today_g)
        cal_year = request.args.get("y", type=int) or ty
        cal_month = request.args.get("m", type=int) or tm
        ...
        if goto_str:
            try:
                ...
                raise ValueError("goto must be YYYY-MM-DD")

        grid = cal["grid"](cal_year, cal_month)
        if not grid:
            raise ValueError("calendar returned an empty month")

        greg_days = [g for g, _, _ in grid]

        # Visibility can be supplied by Flutter; otherwise use web preferences.
        show_param = request.args.getlist("show")
        if show_param:
            show_sources = {s for s in show_param if s in ("bohra", "sunni", "shia")}
            traditions_on = {s for s in show_param if s in ALL_TRADITIONS}
        else:
            show_sources = set()
            if prefs.get("show_bohra", True): show_sources.add("bohra")
            if prefs.get("show_sunni", True): show_sources.add("sunni")
            if prefs.get("show_shia", True): show_sources.add("shia")
            traditions_on = get_visible_traditions(prefs)

        if not show_sources:
            show_sources = {"bohra", "sunni", "shia"}
        if not traditions_on:
            traditions_on = ALL_TRADITIONS - {"bohra", "sunni", "shia"}
        ...
        if selected_str:
            try:
                selected_g = date.fromisoformat(selected_str)
            except ValueError:
                raise ValueError("selected must be YYYY-MM-DD")
        if selected_g is None or selected_g not in greg_days:
            selected_g = today_g if today_g in greg_days else greg_days[0]

        for g, ordinal, label_input in grid:
            personal = personal_by_date.get(g, []) if prefs.get("show_personal", True) else []
            secondary_num = None
            if secondary_obj:
                _, _, sec_label_input = secondary_obj["native_of"](g)
                secondary_num = secondary_obj["native_label"](sec_label_input)
            ...
            events = hijri_events_by_day.get(g, [])
            interfaith = interfaith_by_date.get(g, [])
            all_events = events + interfaith + personal

            days.append({
                "date": g.isoformat(),
                "primary_day": cal["native_label"](label_input),
                "secondary_day": secondary_num,
                "gregorian_day": g.day,
                "gregorian": g.isoformat(),
                "is_today": g == today_g,
                "is_selected": g == selected_g,
                "events": all_events,
                "hijri_events": events,
                "interfaith_events": interfaith,
                "personal_events": personal,
                "is_ekadashi": is_ekadashi,
                "is_purnima": is_purnima,
            })

        first_weekday = (days[0]["date"] and date.fromisoformat(days[0]["date"]).weekday() + 1) % 7
        weeks=[]
        week=[None]*first_weekday
        for d in days:
            week.append(d)
        ...
        greg_start=greg_days[0]; greg_end=greg_days[-1]
        if greg_start.month == greg_end.month:
            gregorian_range=greg_start.strftime("%B %Y")
        else:
            gregorian_range=f"{greg_start.strftime('%B')}/{greg_end.strftime('%B %Y')}"
        ...
        if show_secondary:
            sy1, sm1, _ = secondary_obj["native_of"](greg_start)
            sy2, sm2, _ = secondary_obj["native_of"](greg_end)
            name1=secondary_obj["month_name"](sy1, sm1)
            if (sy1, sm1)==(sy2, sm2):
                secondary_month_label=f"{name1} {sy1}"
            else:
                name2=secondary_obj["month_name"](sy2, sm2)
                secondary_month_label=f"{name1} {sy1} / {name2} {sy2}"
        else:
            secondary_month_label=gregorian_range
        ...
        hebrew_daily=None
        parsi_daily=None
        christian_daily=None
        if cal_key in HIJRI_FAMILY_CALENDARS:
            prayer=pt.calculate(lat, lng, selected_g, tz_offset)
        elif cal_key == "hindu":
            hindu_daily=compute_hindu_daily(today_g, {"lat":lat,"lng":lng,"tz_offset":tz_offset})
        elif cal_key == "hebrew":
            hebrew_daily=compute_hebrew_daily(today_g, {"lat":lat,"lng":lng,"tz_offset":tz_offset})
        elif cal_key == "parsi":
            parsi_daily=compute_parsi_daily(today_g, {"lat":lat,"lng":lng,"tz_offset":tz_offset})
        elif cal_key == "gregorian" and "christian" in traditions_on:
            christian_daily=compute_christian_daily(today_g)

        selected_day=next((d for d in days if d["is_selected"]), days[0])
        selected_date=date.fromisoformat(selected_day["date"])
        selected_events=selected_day["events"]
        ...
        return jsonify({
            ...
            "secondary": {
                "key": secondary_cal if show_secondary else "",
                "label": secondary_obj["label"] if show_secondary else "",
                "month_label": secondary_month_label,
            },
            "location": {
                "name": loc["name"],
                "lat": lat,
            ...
            "today": {
                "date": today_g.isoformat(),
                "events": hijri_events_by_day.get(today_g, []) + interfaith_by_date.get(today_g, []) + (personal_by_date.get(today_g, []) if prefs.get("show_personal", True) else []),
            },
            "prayer": prayer,
            "hindu_daily": hindu_daily,
            "hebrew_daily": hebrew_daily,
            "parsi_daily": parsi_daily,
            "christian_daily": christian_daily,
        })
    ...
    @app.get("/api/mobile/prayer-times")
    def api_mobile_prayer_times():
        """Return prayer times for a rolling date range using the same
        calculator and saved location as the web calendar. This endpoint is
        intended for the Samaa Android WebView so Android can schedule several
        days of native notifications without duplicating prayer calculations.

        Optional query params:
          days: number of days starting today (1..14, default 7)
          lat, lng, tz_offset: explicit location override
        """
        prefs = get_user_prefs()

        # Azaan reminders are intended only for the Hijri-family calendars,
        # matching the web application's existing preference logic.
        if prefs.get("default_calendar", "hijri") not in HIJRI_FAMILY_CALENDARS:
            return jsonify({
                "enabled": False,
                "reason": "default_calendar_not_hijri_family",
                "days": [],
            })

        loc = current_location()
        try:
            lat = float(request.args.get("lat")) if request.args.get("lat") is not None else float(loc["lat"])
            lng = float(request.args.get("lng")) if request.args.get("lng") is not None else float(loc["lng"])
            tz_offset = float(request.args.get("tz_offset")) if request.args.get("tz_offset") is not None else float(loc["tz_offset"])
        except (TypeError, ValueError, KeyError):
            raise ValueError("lat, lng and tz_offset must be valid numbers")
        ...
        for offset in range(days):
            ...
            result.append(pt.calculate(lat, lng, d, tz_offset))
    ...
    def api_mobile_events():
        ...
        today_g = date.today()
        start_g = date.fromisoformat(start_str) if start_str else today_g
        end_g = date.fromisoformat(end_str) if end_str else start_g + timedelta(days=30)
        if end_g < start_g:
            raise ValueError("end must be >= start")
        if (end_g - start_g).days > 90:
            raise ValueError("range too large -- max 90 days per request")

        lat = parse_float("lat", required=False)
        lng = parse_float("lng", required=False)
        tz_offset = parse_float("tz_offset", required=False)

        show_param = request.args.getlist("show")  # ?show=bohra&show=sunni&show=christian ...
        show_sources = {s for s in show_param if s in ("bohra", "sunni", "shia")} or {"bohra", "sunni", "shia"}
        traditions = {s for s in show_param if s in ALL_TRADITIONS - {"bohra", "sunni", "shia"}} or (ALL_TRADITIONS - {"bohra", "sunni", "shia"})

        hijri_by_date = get_hijri_events_for_gregorian_range(start_g, end_g, show_sources)
        interfaith_by_date = get_interfaith_by_date(start_g, end_g, traditions)
        ...
        while d <= end_g:
            ...
            day_payload = {
                "date": d.isoformat(),
                "hijri": {"year": hy, "month": hm, "month_name": hc.hijri_month_name(hm), "day": hd},
                "hijri_events": hijri_by_date.get(d, []),
                "interfaith_events": interfaith_by_date.get(d, []),
            }
            if lat is not None and lng is not None and tz_offset is not None:
                day_payload["prayer_times"] = pt.calculate(lat, lng, d, tz_offset)
            days.append(day_payload)
            d += timedelta(days=1)

        return jsonify({"start": start_g.isoformat(), "end": end_g.isoformat(), "days": days})
